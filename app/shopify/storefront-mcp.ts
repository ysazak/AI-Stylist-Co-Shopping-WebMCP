...
const safeImage = (value: unknown) => { const raw = clean(value, 1000); try { const url = new URL(raw); return url.protocol === "https:" && (url.hostname === "cdn.shopify.com" || url.hostname === "redaifoxes.myshopify.com") ? url.toString() : undefined; } catch { return undefined; } };
const money = (value: unknown): Money | undefined => { const raw = value as Record<string, unknown> | number | string | undefined; const amount = typeof raw === "object" && raw ? first(raw.amount, raw.amount_minor, raw.value) : raw; const currency = typeof raw === "object" && raw ? clean(first(raw.currency, raw.currency_code), 3).toUpperCase() : "EUR"; const numberValue = Number(amount); return Number.isFinite(numberValue) && numberValue >= 0 ? { amountMinor: Math.round(numberValue), currency: currency || "EUR" } : undefined; };
...
function card(raw: unknown, slot: Slot): CatalogCard | undefined {
  ...
  const price = money(first(product.price, product.price_range, product.priceRange));
  return { id, title, slot, image: safeImage(first(product.image, product.featured_image, (product.images as unknown[])?.[0])), price, available: product.available !== false && product.available_for_sale !== false, productType: type || undefined };
}
function detail(raw: unknown, slot: Slot): CatalogProduct | undefined {
  ...
  const variants = asArray(product.variants).map((item): CatalogVariant | undefined => { const variant = item as Record<string, unknown>; const id = clean(first(variant.id, variant.variant_id), 256); if (!id) return undefined; return { id, title: clean(first(variant.title, variant.name), 160) || "Default", available: variant.available !== false && variant.available_for_sale !== false, price: money(first(variant.price, variant.price_amount)), selectedOptions: asArray(first(variant.selected_options, variant.selectedOptions, variant.options)).map((option) => { const value = option as Record<string, unknown>; return { name: clean(first(value.name), 80), value: clean(first(value.value, value.label), 80) }; }).filter((option) => option.name && option.value), image: safeImage(first(variant.image, variant.featured_image)) }; }).filter((item): item is CatalogVariant => Boolean(item));
