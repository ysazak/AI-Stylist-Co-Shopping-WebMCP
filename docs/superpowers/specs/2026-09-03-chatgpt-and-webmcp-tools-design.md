...
- Open the configured URL in a new tab with `noopener,noreferrer` protections.
...
- Open a dedicated popup using the same drawer styling, placement, headings, scroll behavior, and mobile treatment as WebMCP activity.
...
- Define the tool metadata once and use it both for browser registration and the tools popup, preventing drift.
